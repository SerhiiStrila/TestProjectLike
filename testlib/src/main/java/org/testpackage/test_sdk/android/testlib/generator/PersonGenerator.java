...
public class PersonGenerator {
    ...
    private int getIntStatus(int except1, int except2) {
        try {
            ...
            if (res == except1 || res == except2)
                return getIntStatus(except1, except2);
            if (res == 1)
                return (isReally(2)) ? res : getIntStatus(except1, except2);
            if (res == 2)
                return (isReally(12)) ? res : getIntStatus(except1, except2);
            if (res == 3)
                return (isReally(25)) ? res : getIntStatus(except1, except2);
            else
                return res;
        } catch (Exception e) {
            return 3;
